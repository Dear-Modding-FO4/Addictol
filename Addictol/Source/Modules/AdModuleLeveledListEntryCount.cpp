...
namespace Addictol
{
	...
	bool ModuleLeveledListEntryCount::DoInstall([[maybe_unused]] F4SE::MessagingInterface::Message *a_msg) noexcept
	{
		...
		const auto &levItemArray = dataHandler->GetFormArray<RE::TESLevItem>();
		...
		for (const RE::TESLevItem *levItem : levItemArray)
		{
			if (!levItem) { continue; }

			const RE::TESLeveledList *levItemList = levItem->As<RE::TESLeveledList>();
