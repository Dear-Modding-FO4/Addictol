...
namespace Addictol
{
	static REX::TOML::Bool<> bPathesGreyMovie{ "Patches", "bGreyMovie", true };

	static void HKGfxSetBGAlpha(Scaleform::GFx::Movie* self, float)
	{
	...
	bool ModuleGreyMovie::DoInstall(F4SE::MessagingInterface::Message* a_msg) noexcept
	{
		if (!RELEX::IsRuntimeOG())
		{
			...
			Detours::X64::DetourFunction(Target + 0x27, (uintptr_t)&HKGfxSetBGAlpha, Detours::X64Option::USE_REL32_CALL);
		}
		else
		{
			// OG
