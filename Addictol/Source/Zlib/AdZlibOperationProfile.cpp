...
namespace Addictol
{
	...
	OperationProfileSource* ZlibOperationProfile() noexcept { return s_zlibProfile.Get(); }
	...
	bool InitializeZlibOperationProfile(TelemetryHub& a_hub, ZlibBackendKind a_backend) noexcept
	{
		...
		{
			if (backend.kind != a_backend) continue;
			const std::array labels{
				OperationProfileMetadataLabel{ "selected_backend", backend.name },
				OperationProfileMetadataLabel{ "whole_decoder", backend.whole },
				OperationProfileMetadataLabel{ "streaming_decoder", backend.streaming }
			};
