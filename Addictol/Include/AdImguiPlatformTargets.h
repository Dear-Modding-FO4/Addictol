...
namespace Addictol::ImguiPlatform
{
	...
	inline constexpr std::array<uint8_t, 42> kUIEndFrameSignatureOG{
	...
	inline constexpr std::array<uint8_t, 39> kUIEndFrameSignatureNG{
	...
	inline constexpr std::array<uint8_t, 39> kUIEndFrameSignatureAE{
		0x48, 0x89, 0x5C, 0x24, 0x08, 0x48, 0x89, 0x6C, 0x24, 0x10, 0x48, 0x89, 0x74, 0x24, 0x18,
		0x48, 0x89, 0x7C, 0x24, 0x20, 0x41, 0x54, 0x41, 0x56, 0x41, 0x57, 0x48, 0x83, 0xEC, 0x20,
		0x8B, 0x15, 0xE4, 0xA7, 0x3D, 0x02, 0x4C, 0x8B, 0xF9
	};

	[[nodiscard]] constexpr std::span<const uint8_t> UIEndFrameSignature(Runtime a_runtime) noexcept
	{
		switch (a_runtime)
		{
		case Runtime::kOG:
	...
	[[nodiscard]] constexpr bool MatchesSignature(
		std::span<const uint8_t> a_candidate,
		std::span<const uint8_t> a_signature) noexcept
	{
		if (a_signature.empty() || a_candidate.size() != a_signature.size())
			return false;

		for (size_t index = 0; index < a_signature.size(); ++index)
		{
			if (a_candidate[index] != a_signature[index])
				return false;
		}
		return true;
	}
	...
	{
		return a_message == kKeyDownMessage && !IsKeyRepeat(a_lparam);
	}
}
