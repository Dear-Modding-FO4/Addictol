...
namespace F4SE
{
	...
	constexpr static uint32_t kPluginHandle_Invalid = 0xFFFFFFFFul;

	enum class QueryInterface : uint32_t
	{
		kInvalid = 0,
		kMessaging,
		kScaleform,
	...
	struct F4SEInterface
	{
		...
		uint32_t isEditor;
		void* (*QueryInterface)(QueryInterface);
		...
		const PluginInfo* (*GetPluginInfo)(const char*);
	...
	struct MessagingInterface
	{
		...
		constexpr static uint32_t kInterfaceVersion = 1;
		
		uint32_t interfaceVersion{ kInterfaceVersion };
		...
		void* (*GetEventDispatcher)(uint32_t);
	};
	...
	struct ScaleformInterface
	{
		...
		typedef bool (*RegisterCallback)(void*/*GFxMovieView**/, void*/*GFxValue**/);
	...
	{
		constexpr static uint32_t kInterfaceVersion = 1;
		typedef void (*EventCallback)(const SerializationInterface*);
		typedef void (*FormDeleteCallback)(uintptr_t);

		uint32_t interfaceVersion{ kInterfaceVersion };

		void (*SetUniqueID)(PluginHandle, uint32_t);

		void (*SetRevertCallback)(PluginHandle, EventCallback);
		void (*SetSaveCallback)(PluginHandle, EventCallback);
		void (*SetLoadCallback)(PluginHandle, EventCallback);
		void (*SetFormDeleteCallback)(PluginHandle, FormDeleteCallback);

		bool (*WriteRecord)(uint32_t, uint32_t, const void*, uint32_t);
		bool (*OpenRecord)(uint32_t, uint32_t);
		bool (*WriteRecordData)(const void*, uint32_t);

		bool (*GetNextRecordInfo)(uint32_t*, uint32_t*, uint32_t*);
		uint32_t (*ReadRecordData)(void*, uint32_t);
		bool (*ResolveHandle)(uintptr_t, uintptr_t*);
		bool (*ResolveFormId)(uint32_t, uint32_t*);
	};
