...
namespace Addictol::ImguiPlatform
{
	...
	[[nodiscard]] constexpr EngineCursorPosition ResolveEngineCursorPosition(
		...
		uint32_t a_backBufferHeight) noexcept
	{
		const auto useEnginePosition =
			a_enginePositionAvailable &&
			IsInsideDisplay(
				a_engineClientPosition,
				a_clientWidth,
				a_clientHeight);
		const auto clientPosition = useEnginePosition ?
			a_engineClientPosition :
			a_absoluteClientPosition;
		return {
			MapClientToBackBuffer(
				clientPosition,
				a_clientWidth,
				a_clientHeight,
				a_backBufferWidth,
