#include "../Addictol/Include/AdZlibInflate.h"
#include "Harness.h"
...
namespace
{
	...
	{
		const auto write = [&a_code](std::size_t a_offset, const auto& a_bytes) {
			std::copy(a_bytes.begin(), a_bytes.end(), a_code.begin() + a_offset);
		};

		write(0x0, std::array<std::uint8_t, 30>{
		...
		write(0x78, std::array<std::uint8_t, 4>{ 0x41, 0x8B, 0x45, 0x00 });
		write(0xA5, std::array<std::uint8_t, 3>{ 0x83, 0xF8, 0x1E });
		write(0x1569, std::array<std::uint8_t, 8>{ 0x41, 0xC7, 0x45, 0x00, 0x1C, 0x00, 0x00, 0x00 });
		write(0x1CBE, std::array<std::uint8_t, 3>{ 0x45, 0x33, 0xC0 });
		write(0x1CE5, std::array<std::uint8_t, 7>{ 0x4C, 0x89, 0x02, 0x44, 0x89, 0x42, 0x0C });
	}
