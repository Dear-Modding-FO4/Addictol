...
namespace
{
	...
	constexpr uint8_t expected_modern_entry[]{
	...
	constexpr uint8_t expected_og_dispatcher[]{
	...
	constexpr uint8_t expected_og_resident_inner[]{
	...
	constexpr uint8_t expected_og_alternate_inner[]{
		0x48, 0x89, 0x5C, 0x24, 0x08, 0x48, 0x89, 0x6C, 0x24, 0x10,
...
namespace vmm_tests
{
	void run_texture_stream_checks(Runner& runner)
	{
		...
		runner.test("texture seam guards stay exact and relocation free", [] {
			...
			const std::initializer_list<uint8_t> modernOverwrite{ 0x40, 0x53, 0x57, 0x41, 0x56 };
			const std::initializer_list<uint8_t> ogOverwrite{ 0x48, 0x8B, 0x41, 0x18, 0x48, 0x85, 0xC0 };
			require(
			...
			const std::initializer_list<uint8_t> residentReset{ 0xC7, 0x42, 0x44, 0x00, 0x00, 0x00, 0x00 };
			require(
				std::equal(residentReset.begin(), residentReset.end(), Guards::OG_RESIDENT_INNER.begin() + 23),
				"the OG resident entry reset moved");
			const std::initializer_list<uint8_t> modernReset{ 0xC7, 0x41, 0x44, 0x00, 0x00 };
			require(
				std::equal(modernReset.begin(), modernReset.end(), Guards::MODERN_ENTRY.begin() + 28),
				"the AE/NG entry reset moved");
		});

		runner.test("texture guard bytes are pinned independently", [] {
			require(std::ranges::equal(expected_modern_entry, Guards::MODERN_ENTRY),
				"AE/NG entry guard bytes changed");
			require(std::ranges::equal(expected_og_dispatcher, Guards::OG_DISPATCHER),
				"OG dispatcher guard bytes changed");
			require(std::ranges::equal(expected_og_resident_inner, Guards::OG_RESIDENT_INNER),
				"OG resident guard bytes changed");
			require(std::ranges::equal(expected_og_alternate_inner, Guards::OG_ALTERNATE_INNER),
				"OG alternate guard bytes changed");
		});

		runner.test("texture caller bytes are pinned independently", [] {
			require(std::ranges::equal(expected_modern_pre, CallerBytes::MODERN_PRE),
				"AE/NG caller prefix bytes changed");
			require(std::ranges::equal(expected_modern_streaming_post, CallerBytes::MODERN_STREAMING_POST),
				"AE/NG streaming caller suffix bytes changed");
			require(std::ranges::equal(expected_modern_array_post, CallerBytes::MODERN_ARRAY_POST),
				"AE/NG array caller suffix bytes changed");
			require(std::ranges::equal(expected_og_pre, CallerBytes::OG_PRE),
				"OG caller prefix bytes changed");
			require(std::ranges::equal(expected_og_streaming_post, CallerBytes::OG_STREAMING_POST),
				"OG streaming caller suffix bytes changed");
			require(std::ranges::equal(expected_og_array_post, CallerBytes::OG_ARRAY_POST),
				"OG array caller suffix bytes changed");
		});
		...
		runner.test("zlib member headers are validated without reading private state", [] {
			...
			const std::initializer_list<uint8_t> tooShort{ 0x78 };
