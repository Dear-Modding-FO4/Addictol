...
namespace vmm_tests
{
	void run_imgui_platform_checks(Runner& runner)
	{
		...
		runner.test("virtual cursor opening seeds once and resets pending raw motion", [] {
			...
			state.synchronizedFromEngine = true;
			...
			require(!state.rawReport.hasAbsolutePosition &&
					!state.rawInputObserved &&
					!state.synchronizedFromEngine,
				"opening retained stale raw or engine state");
		});
		...
		runner.test("modal cursor source follows the Fallout cursor menu", [] {
			require(
				DecideModalCursorSource(false, false) == ModalCursorSource::kInactive &&
					DecideModalCursorSource(false, true) == ModalCursorSource::kInactive,
				"an overlay-only frame selected a cursor source");
			require(
				DecideModalCursorSource(true, false) == ModalCursorSource::kVirtual,
				"normal gameplay did not select raw virtual motion");
			require(
				DecideModalCursorSource(true, true) == ModalCursorSource::kEngine,
				"an open Fallout cursor menu did not select the engine position");

			require(
				DecideVirtualCursorPositionSource(false, false) ==
					VirtualCursorPositionSource::kAbsoluteFallback,
				"a session without raw reports did not retain absolute fallback");
			require(
				DecideVirtualCursorPositionSource(true, false) ==
					VirtualCursorPositionSource::kRawMotion,
				"an observed raw report did not select virtual cursor motion");
			require(
				DecideVirtualCursorPositionSource(false, true) ==
					VirtualCursorPositionSource::kSynchronizedEngine,
				"an engine handoff did not preserve the synchronized position");
		...
		runner.test("every cursor source transition resets pending raw motion", [] {
			constexpr std::array sources{
				ModalCursorSource::kInactive,
			...
			for (const auto from : sources)
			{
				for (const auto to : sources)
				{
					VirtualCursorState state{};
					OpenVirtualCursor(state, { 100.0f, 100.0f }, 1920, 1080);
					state.accumulated = { 12, -8 };
					state.rawReport = { true, 400, 300 };
					auto current = from;
					TransitionModalCursorSource(state, current, to);
					require(current == to, "a cursor source transition did not take effect");
					if (from == to)
					{
						require(
							state.accumulated.x == 12 &&
								state.accumulated.y == -8 &&
								state.rawReport.hasAbsolutePosition,
							"a steady source discarded current motion");
					}
					else
					{
						require(
							state.accumulated.x == 0 &&
								state.accumulated.y == 0 &&
								!state.rawReport.hasAbsolutePosition,
							"a cursor source transition retained buffered motion");
					}
				}
			}
		});

		runner.test("engine frames synchronize the virtual cursor for a seamless handoff", [] {
			VirtualCursorState state{};
			OpenVirtualCursor(state, { 100.0f, 100.0f }, 1920, 1080);
			state.accumulated = { 50, -20 };
			state.rawReport = { true, 900, 700 };
			const auto synchronized =
				SynchronizeVirtualCursor(state, { 640.0f, 360.0f }, 1920, 1080);
			require(synchronized.x == 640.0f && synchronized.y == 360.0f,
				"the virtual cursor did not follow the visible engine cursor");
			require(state.accumulated.x == 0 &&
					state.accumulated.y == 0 &&
					!state.rawReport.hasAbsolutePosition &&
					state.synchronizedFromEngine,
				"engine synchronization retained stale raw motion");

			ModalCursorSource source{ ModalCursorSource::kEngine };
			TransitionModalCursorSource(state, source, ModalCursorSource::kVirtual);
			const auto resumed =
				ConsumeVirtualCursorMotion(state, 1920, 1080);
			require(resumed.x == 640.0f && resumed.y == 360.0f,
				"closing the Fallout cursor menu moved the virtual cursor");
			require(
				DecideVirtualCursorPositionSource(
					state.rawInputObserved,
					state.synchronizedFromEngine) ==
					VirtualCursorPositionSource::kSynchronizedEngine,
				"the virtual branch discarded the handoff before raw motion resumed");
		});

		runner.test("engine menu cursor maps from client pixels into the backbuffer", [] {
			constexpr auto equal = ResolveEngineCursorPosition(
				true,
				{ 123.75f, 456.25f },
				{ 10.0f, 20.0f },
				1920,
			...
			constexpr auto scaled = ResolveEngineCursorPosition(
				...
				1080);
			require(
				scaled.source == EngineCursorPositionSource::kEngine &&
		...
		runner.test("invalid engine menu positions fall back to the absolute client cursor", [] {
			constexpr auto negative = ResolveEngineCursorPosition(
				true,
				{ -1.0f, 270.0f },
				{ 240.0f, 135.0f },
				960,
				...
				1080);
			require(
				negative.source == EngineCursorPositionSource::kAbsoluteFallback &&
			...
			constexpr auto unavailable =
				-(std::numeric_limits<float>::max)();
			constexpr auto sentinel = ResolveEngineCursorPosition(
				true,
				...
				1200);
			require(
				sentinel.source == EngineCursorPositionSource::kAbsoluteFallback &&
					sentinel.position.x == 240.0f &&
					sentinel.position.y == 160.0f,
				"the engine sentinel did not use the absolute fallback");

			constexpr auto noSingleton = ResolveEngineCursorPosition(
				false,
				{ 300.0f, 200.0f },
				{ 100.0f, 50.0f },
				800,
				...
				1200);
			require(
				noSingleton.source == EngineCursorPositionSource::kAbsoluteFallback &&
